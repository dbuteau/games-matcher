...
class Import(commands.Cog, name='Library Management'):
        ...
        try:
            ...
            if gameInfos is not None:
                ...
                if gameInfos['release_date']['date'] != '':
                    ...
                    """
                    for fmt in ('%d %b, %Y', '%b %d, %Y', '%d %b %Y', '%d/%b/%Y', '%b %Y'):
                        try:
                            result['release_date'] = datetime.datetime.strptime(gameInfos['release_date']['date'], fmt)
                            return result
                        except ValueError:
                            self.logger.info(f"[{gameInfos['name']}#{steam_id}] {gameInfos['release_date']['date']} not parsed by {fmt}")
                            pass
                    """
                else:
    ...
    async def steam(self, ctx, user_steam_id):
        """ <steam_id> Import the steam_id(account numeric id) library """
        try:
            self.logger.info(f'start Import of steam lib for {ctx.author.id}')
            await ctx.bot.change_presence(activity=discord.Activity(type=discord.ActivityType.playing, name=f"importing steam lib"))
            ...
            embed.add_field(name=f'Total Games found', value=raw_data['response']['game_count'])
    ...
    async def forceupdate(self, ctx, criteria: str):
        try:
            if criteria == 'date':
                query = self._db.query(Games).filter(Games.multiplayer.is_(True), Games.release_date == None, Games.steam_id != None)
            elif criteria == 'multi':
                query = self._db.query(Games).filter(Games.multiplayer.is_(True), Games.steam_id != None, or_(Games.release_date == None, Games.release_date.is_(False)))
            else:
                ...
                for oGame in query.all():
                    ...
                    if details is not None:
                        ...
                        if 'multiplayer' in details:
                            oGame.multiplayer = details['multiplayer']
                        self._db.commit()
                        self.logger.debug(f'{oGame.name} updated with release_date: {oGame.release_date} multiplayer: {oGame.multiplayer}')
        except Exception as err:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            self.logger.error(f'{fname}({exc_tb.tb_lineno}): {err}')
    ...
    @library.command(name='import gog', hidden=False)
    @commands.max_concurrency(1, per=BucketType.default, wait=True)
    @commands.dm_only()
